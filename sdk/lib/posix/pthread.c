...
static struct pthread_task *pthread_get(pthread_t thread)
{
    int32 i = 0;
...
int pthread_create(pthread_t *thread,      const pthread_attr_t *attr, void *(*start)(void *), void *arg)
{
    ...
    OS_TASK_INIT(thd->name, &thd->task, pthread_task_entry, thd, pthread_priority(&thd->attr), thd->attr.stacksize);
...
{
    struct os_task *task = os_task_current();
    struct pthread_task *thd = container_of(task, struct pthread_task, task);
    return (pthread_t)pthread_get((pthread_t)thd);
}
...
uint32 pthread_timespec_delta(const struct timespec *abstime)
{
    uint64 ticks;
