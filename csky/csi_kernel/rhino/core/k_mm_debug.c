        ...
        while (cur) {
            ...
            if ((cur->size & RHINO_MM_BLKSIZE_MASK)) {
                ...
                if (0 == g_recheck_flag && !(cur->size & RHINO_MM_FREE)) {
                    if ((uint8_t *)krhino_cur_task_get()->task_stack_base >= cur->mbinfo.buffer && (uint8_t *)krhino_cur_task_get()->task_stack_base < (uint8_t *)next) {
                        cur = next;
                        continue;
                    }
    ...
    while (reginfo) {
        ...
        while (cur) {
            ...
            if ((cur->size & RHINO_MM_BLKSIZE_MASK)) {
                next = NEXT_MM_BLK(cur->mbinfo.buffer, cur->size & RHINO_MM_BLKSIZE_MASK);
                if (!(cur->size & RHINO_MM_FREE) &&
                    0 == check_mm_leak(cur->mbinfo.buffer) && 0 == recheck((void *)cur->mbinfo.buffer, (void *)next)) {
