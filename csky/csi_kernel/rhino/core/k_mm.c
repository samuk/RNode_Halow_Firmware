...
#define ISFIXEDBLK(mh, ptr) \
    (mh->fixedmblk && ((void *)ptr > (void *)(mh->fixedmblk->mbinfo.buffer)) && ((void *)ptr < (void *)(mh->fixedmblk->mbinfo.buffer + mh->fixedmblk->size))) ? 1 : 0
...
kstat_t krhino_init_mm_head(k_mm_head **ppmmhead, void *addr, size_t len) {
    ...
    */
    ...
    if (!len || len < MIN_FREE_MEMORY_SIZE + RHINO_CONFIG_MM_TLF_BLK_SIZE || len > MAX_MM_SIZE) {
...
#if defined(__VALGRIND_MAJOR__) && defined(__VALGRIND_MINOR__) && (__VALGRIND_MAJOR__ > 3 || (__VALGRIND_MAJOR__ == 3 && __VALGRIND_MINOR__ >= 12))
    ...
    while (ptr) {
        ...
        if ((unsigned long)ib1 == (unsigned long)lb0 + MMLIST_HEAD_SIZE) {
            ...
            b0->size = MM_ALIGN_DOWN((b0->size & RHINO_MM_BLKSIZE_MASK) +
                                     (ib1->size & RHINO_MM_BLKSIZE_MASK) + 2 * MMLIST_HEAD_SIZE) |
        ...
        if ((unsigned long)lb1->mbinfo.buffer == (unsigned long)ib0) {
            ...
            lb1->size = MM_ALIGN_DOWN((b0->size & RHINO_MM_BLKSIZE_MASK) +
                                      (ib0->size & RHINO_MM_BLKSIZE_MASK) + 2 * MMLIST_HEAD_SIZE) |
                        RHINO_MM_ALLOCED | (lb1->size & RHINO_MM_PRESTAT_MASK);
...
void *k_mm_alloc(k_mm_head *mmhead, size_t size) {
    ...
    if (retptr != NULL) {
        stats_addsize(mmhead, ((b->size & RHINO_MM_BLKSIZE_MASK) + MMLIST_HEAD_SIZE), req_size);
    }
...
void *k_mm_realloc(k_mm_head *mmhead, void *oldmem, size_t new_size) {
    ...
    cpsize = ((b->size & RHINO_MM_BLKSIZE_MASK) > new_size) ? new_size : (b->size & RHINO_MM_BLKSIZE_MASK);
